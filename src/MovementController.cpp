...
bool MovementController::BeginCurrentNode(ActiveRuntimeMovement& movement)
{
    ...
    for (RuntimeEntity const& entity : group->Entities)
    {
        ...
        {
            // Diagnostic route behavior: launch the exact MMAP-generated point
            // array directly as a MoveSpline. MovebyPath is linear unless
            // SetSmooth() is explicitly enabled, which we intentionally do not
            // do here. This bypasses EscortMovementGenerator while preserving
            // the MMAP-generated path itself.
            ...
            {
                LOG_INFO("server.loading",
                    "[LWI Route Debug] Group #{} path {} node {} launched direct LINEAR MoveSpline with {} MMAP point(s), bypassing EscortMovementGenerator.",
                    movement.RuntimeGroupId,
                    movement.PathId,
                    node.NodeOrder,
                    pathPoints.size());
        ...
        {
            // Existing invasion movement remains unchanged.
...
void MovementController::ResumeInterruptedCreatures(ActiveRuntimeMovement& movement)
{
    ...
    for (RuntimeMovementDestination& destination : movement.Destinations)
    {
        ...
        if (movement.RouteMovement)
        {
            ...
            {
                LOG_INFO("server.loading",
                    "[LWI Route Debug] Group #{} path {} node {} resumed with direct LINEAR MoveSpline using {} MMAP point(s), bypassing EscortMovementGenerator.",
                    movement.RuntimeGroupId,
                    movement.PathId,
                    node.NodeOrder,
                    pathPoints.size());
